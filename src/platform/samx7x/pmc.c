...
{
	if (div)
		PMC->PMC_USB = PMC_USB_USBDIV(div - 1);
}

void pmc_switch_udpck_to_upllck(u32 div)
{
	if (div)
		PMC->PMC_USB = PMC_USB_USBS | PMC_USB_USBDIV(div - 1);
}
...
void pmc_generic_clock_config(u8 id, u8 enable, u32 source, u16 prescaler)
{
	...
	PMC->PMC_PCR = (PMC->PMC_PCR & ~(PMC_PCR_GCLKDIV_Msk | PMC_PCR_GCLKCSS_Msk)) | PMC_PCR_GCLKEN_Msk |
		       (source & PMC_PCR_GCLKCSS_Msk) | PMC_PCR_GCLKDIV(prescaler);
}
