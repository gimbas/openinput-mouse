 ...
 */
#define _PIO(port) ((Pio *) (PIOA + (port * 0x00000200UL)))
...
{
	pio_direction(pin, config.direction);
	pio_pull(pin, config.pull);
	...
	pio_set(pin, config.out_data);
	pio_peripheral_mux(pin, config.mux);
	pio_peripheral_control(pin, config.peripheral_control);
}
